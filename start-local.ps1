...
$prowlarrBaseUrl = [string]$config.providers.prowlarr.base_url
...
if ($providerName -eq "prowlarr" -and $prowlarrBaseUrl) {
  $prowlarrUri = [Uri]$prowlarrBaseUrl
  $prowlarrPort = if ($prowlarrUri.IsDefaultPort) {
    if ($prowlarrUri.Scheme -eq "https") { 443 } else { 80 }
  } else {
    $prowlarrUri.Port
  }

  Write-Host "Checking Prowlarr at $prowlarrBaseUrl..."
  try {
    Test-TcpEndpoint -HostName $prowlarrUri.Host -Port $prowlarrPort | Out-Null
  } catch {
    throw "Prowlarr is configured at $prowlarrBaseUrl but is not reachable. Start Prowlarr first or update config.local.json. Details: $($_.Exception.Message)"
  }
}
